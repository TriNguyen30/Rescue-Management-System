  ...
  const saveInfo = async () => {
    ...
    try {
      if (role !== user.role) {
        ...
        onSaved(updated);
        success("Vai trò đã được cập nhật", `${user.fullName || user.username} → ${role}`);
      } else {
        onSaved({ ...user });
            ...
            <>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">
                  Vai trò
                  {role !== user.role && (
                    <span className="ml-2 text-xs font-normal text-amber-600 bg-amber-50 border border-amber-200 px-1.5 py-0.5 rounded-md">Đã thay đổi</span>
                  )}
                </label>
...
function DeleteConfirm({ user, onClose, onDeleted }: { user: User; onClose: () => void; onDeleted: (id: string) => void }) {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      onDeleted(user.id);
      success("Đã xóa người dùng", `${user.fullName || user.username} đã bị xóa khỏi hệ thống.`);
      onClose();
    } catch (e: any) {
    ...
    <tr className="group hover:bg-blue-50/40 transition-colors">
      ...
      <td className="px-4 py-3">
        <div className="flex items-center gap-1 justify-end">
          <button onClick={() => onEdit(user)} className="p-1.5 rounded-lg hover:bg-blue-100 text-gray-400 hover:text-blue-600 transition-colors cursor-pointer" title="Chỉnh sửa">
            <Edit3 className="w-4 h-4" />
          </button>
          <button onClick={() => onDelete(user)} className="p-1.5 rounded-lg hover:bg-red-100 text-gray-400 hover:text-red-500 transition-colors cursor-pointer" title="Xóa">
...
// ── Inner dashboard (consumes PageContext) ────────────────────────────────────
function UserDashboardInner() {
  const { error: toastError, info } = useToast();
  const { page, pageSize, totalPages, goToPage, setPage, setTotalItems } = usePage();
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      ...
      const msg = e?.response?.data?.message || "Không thể tải danh sách người dùng.";
      setError(msg);
  ...
  // Keep PageContext in sync with filtered count; reset to page 1 when filters change
  useEffect(() => {
    setTotalItems(filtered.length);
  }, [filtered.length, setTotalItems]);

  // Keep page index valid after filtering/deleting changes total pages.
  useEffect(() => {
    if (page > totalPages) {
      goToPage(totalPages);
    }
          ...
          {stats.map(({ role, count }) => {
            ...
            return (
              <button key={role} onClick={() => setRoleFilter(roleFilter === role ? "ALL" : role)}
                className={`p-4 rounded-2xl border text-left transition-all cursor-pointer ${roleFilter === role ? `${meta.bg} ${meta.color} border-current shadow-sm` : "bg-white border-gray-100 hover:border-gray-200 text-gray-700"}`}>
          ...
          {!loading && !error && filtered.length > 0 && (
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3 px-4 py-3 border-t border-gray-100">
              <p className="text-xs text-gray-400 shrink-0">
                Hiển thị {Math.min((page - 1) * pageSize + 1, filtered.length)}–{Math.min(page * pageSize, filtered.length)} / {filtered.length} người dùng
              </p>
