    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
        ...
        <div className="p-6 space-y-4">
            ...
            <>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">
                  Vai trò
                  {role !== user.role && (
                    <span className="ml-2 text-xs font-normal text-amber-600 bg-amber-50 border border-amber-200 px-1.5 py-0.5 rounded-md">
                      Đã thay đổi
                    </span>
                  )}
                </label>
          ...
          <button
            onClick={tab === "info" ? saveInfo : savePassword}
            disabled={saving}
    ...
    <tr className="group hover:bg-blue-50/40 transition-colors">
      <td className="px-4 py-3">
        <div className="flex items-center gap-3">
          <div
            className={`w-9 h-9 rounded-xl flex items-center justify-center text-sm font-bold shrink-0 ${colors[colorIdx]}`}
          >
            {initials}
          </div>
          <div>
