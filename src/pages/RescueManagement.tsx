      ...
      {searchable && options.length > 4 && (
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 text-sm border border-gray-200 rounded-xl focus:ring-2 focus:blue-emerald-500 focus:border-transparent outline-none bg-gray-50 focus:bg-white transition"
...
function EditRescueTeamModal({
  ...
  const handlePasteMapLink = (url: string) => {
    ...
    if (!lat && qMatch) { lat = qMatch[1]; lng = qMatch[2]; }

    const rawMatch = url.match(/^\s*(-?\d+\.\d+)\s*,\s*(-?\d+\.\d+)\s*$/);
    if (!lat && rawMatch) { lat = rawMatch[1]; lng = rawMatch[2]; }

    if (lat && lng) { setLatitude(lat); setLongitude(lng); }
  };
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="space-y-3">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">Dán link Google Maps</label>
              <input type="text"
          ...
          <div>
            ...
            <div className="relative">
              <select id="edit_leader_id" value={leaderId}
                onChange={(e) => { const next = e.target.value; setLeaderId(next); setMemberIds((prev) => prev.filter((id) => id !== next)); }}
                className="w-full px-3 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none bg-gray-50 focus:bg-white appearance-none transition cursor-pointer">
          ...
          <CheckboxMultiSelect id="edit_member_ids" label="Thành viên" hint="(tick để chọn nhiều người)"
            options={userOptions.filter((o) => o.value !== leaderId)} value={memberIds} onChange={setMemberIds} />

          <CheckboxMultiSelect id="edit_vehicle_ids" placeholder="Tìm theo biển số xe..." label="Phương tiện"
            hint="(tick để chọn nhiều xe)" options={vehicleOptions} value={vehicleIds} onChange={setVehicleIds} />
...
export default function RescueManagement() {
  ...
  const filteredIds = useMemo(() => filtered.map(resolveTeamId), [filtered]);
  ...
  const handleBulkDelete = async () => {
    ...
    setTeams((prev) => prev.filter((t) => !selectedIds.has(resolveTeamId(t)) || failed.includes(resolveTeamId(t))));
    clearSelection();
    setBulkDeleteOpen(false);
    setBulkDeleting(false);

    if (failed.length === 0) {
      toastSuccess(`Đã xóa ${ids.length} đội cứu hộ thành công.`);
    } else {
      toastError(`Xóa thành công ${ids.length - failed.length}/${ids.length} đội. ${failed.length} đội xóa thất bại.`);
    }
    fetchTeams();
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      for (const userId of desiredUsers) {
        try { await assignMemberToRescueTeam(memberAssignPayload(teamId, userId)); } catch { /* ignore */ }
      }
      for (const vehicleId of form.vehicleIds.filter(Boolean)) {
        try { await assignVehicleToRescueTeam(vehicleAssignPayload(teamId, vehicleId)); } catch { /* ignore */ }
      }
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 sm:p-6 lg:p-8">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {selectedIds.size > 0 && (
          <div className="flex items-center justify-between gap-3 px-4 py-3 bg-red-50 border border-red-100 rounded-2xl shadow-sm">
            <div className="flex items-center gap-2 text-sm text-red-700 font-medium">
              <Check className="w-4 h-4 text-red-500" />
              Đã chọn <span className="font-bold">{selectedIds.size}</span> đội cứu hộ
            </div>
            <div className="flex items-center gap-2">
              <button type="button" onClick={clearSelection}
                className="px-3 py-1.5 text-xs font-semibold text-gray-600 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors cursor-pointer">
                Bỏ chọn tất cả
              </button>
              <button type="button" onClick={() => setBulkDeleteOpen(true)}
                className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-white bg-red-500 hover:bg-red-600 rounded-xl transition-colors cursor-pointer">
                <Trash2 className="w-3.5 h-3.5" />
                Xóa {selectedIds.size} đội đã chọn
              </button>
            </div>
          </div>
        )}
                  ...
                  <tr className="border-b border-gray-100 bg-gray-50/60">
                    ...
                    <th className="pl-4 pr-2 py-3 w-10">
                      <input
                        ...
                        className="h-4 w-4 rounded border-gray-300 text-red-500 focus:ring-red-400 focus:ring-offset-0 cursor-pointer"
                      />
                    </th>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label htmlFor="latitude" className="block text-sm font-medium text-gray-700 mb-1.5">Vĩ độ (Latitude)</label>
                  <input id="latitude" type="number" step="any" value={form.latitude}
                    onChange={(e) => setForm((prev) => ({ ...prev, latitude: e.target.value }))}
                    onBlur={handleGeocode} placeholder="VD: 10.7769"
                    className="w-full px-3 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none bg-gray-50 focus:bg-white transition" />
                </div>
                <div>
                  <label htmlFor="longitude" className="block text-sm font-medium text-gray-700 mb-1.5">Kinh độ (Longitude)</label>
                  <input id="longitude" type="number" step="any" value={form.longitude}
                    onChange={(e) => setForm((prev) => ({ ...prev, longitude: e.target.value }))}
                    onBlur={handleGeocode} placeholder="VD: 106.7009"
                    className="w-full px-3 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none bg-gray-50 focus:bg-white transition" />
                </div>
              </div>
              <div>
              ...
              <CheckboxMultiSelect id="member_ids" label="Thành viên" hint="(tick để chọn nhiều người)"
                options={userOptions.filter((o) => o.value !== form.leaderId)}
                value={form.memberIds} onChange={(vals) => handleMultiChange("memberIds", vals)} />
              <CheckboxMultiSelect id="vehicle_ids" placeholder="Tìm theo biển số xe..." label="Phương tiện"
                hint="(tick để chọn nhiều xe)" options={vehicleOptions}
                value={form.vehicleIds} onChange={(vals) => handleMultiChange("vehicleIds", vals)} />
              {formError && (
                <div className="flex items-center gap-2 text-sm text-red-600 bg-red-50 border border-red-100 px-3 py-2 rounded-xl">
      ...
      <Modal
        ...
        title="Xác nhận xóa nhiều đội cứu hộ"
        size="sm"
      >
        <div className="space-y-4">
          <p className="text-sm text-gray-600">
            Bạn có chắc muốn xóa <span className="font-semibold text-gray-900">{selectedIds.size} đội cứu hộ</span> đã chọn?
          ...
          <div className="flex justify-end gap-2">
            ...
            <button type="button" onClick={handleBulkDelete} disabled={bulkDeleting}
              ...
              {bulkDeleting ? "Đang xóa..." : `Xóa ${selectedIds.size} đội`}
            </button>
          </div>
        </div>
      </Modal>
                  ...
                  {selectedTeam.currentLocation?.coordinates?.length === 2 && (
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <button type="button"
                        ...
                        <MapPin className="w-3.5 h-3.5" /> Bản đồ hệ thống
                      </button>
                    </div>
                  )}
                </div>
